...
function tas_watchdog() {
    if(tas_state == 'init') {
        ...
        if(upload_client) {
            ...
            tas_state = 'init_sensor';
        }
    }
    else if(tas_state == 'init_sensor') {
...
function saveLastestData() {
    sensor.read(11, 27, function(err, temp, humi){
        if(!err){
            console.log('sensor value : '+"temp/"+temp+",humi/"+humi);
            create_send_packet(temp, humi);
        }
    });
}
...
function create_send_packet(temp,humi)
{
    send_packet="temp/"+temp+",humi/"+humi
...
function upload_action(data)
{
    if (tas_state == 'upload') {
        var con = {value: 'TAS' + data};
            var cin = {ctname: 'cnt-dht', con: data};
